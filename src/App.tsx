...
function App() {
  ...
  const calculateFare = useCallback((distanceKm: number, waitingMinutes: number, sorianaBonus: boolean = false) => {
    ...
    if (selectedTripType.fixedPrice || (selectedTripType.id === 'cristoRey' && selectedSubTrip)) {
      ...
      return costoAcumuladoParadas + baseCost + waitingCost + sorianaExtraFee + petExtraFee + servicioEspecialFee;
    }
    ...
    return costoAcumuladoParadas + fare + (waitingMinutes * RATES.waitingRate) + sorianaExtraFee + petExtraFee + servicioEspecialFee;
                        ...
                        if (selectedTripType.fixedPrice || (selectedTripType.id === 'cristoRey' && selectedSubTrip)) {
                          ...
                          if (lastTripSummary.distance > baseDistance) {
                            if (selectedTripType.id === 'cristoRey') {
                              ...
                              const extraKm = lastTripSummary.distance - baseDistance;
                              baseCost += extraKm * 16;
                            }
                          }
                        } else {
                          baseCost = RATES.baseFare;
                          for (const rate of RATES.distanceRates) {
                            if (lastTripSummary.distance >= rate.min && lastTripSummary.distance <= rate.max) {
                              if (rate.extraRate && lastTripSummary.distance > 8) {
                                ...
                                baseCost = rate.basePrice! + (extraKm * rate.extraRate);
                              } else {
                                baseCost = rate.price!;
                              }
                              break;
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-bold text-2xl text-green-400">
                          ${(() => {
                            ...
                            if (selectedTripType.fixedPrice || (selectedTripType.id === 'cristoRey' && selectedSubTrip)) {
                              ...
                              if (lastTripSummary.distance > baseDistance) {
                                if (selectedTripType.id === 'cristoRey') {
                                  ...
                                  const extraKm = lastTripSummary.distance - baseDistance;
                                  distanceCost += extraKm * 16;
                                }
                              }
                            } else {
                              distanceCost = RATES.baseFare;
                              for (const rate of RATES.distanceRates) {
                                if (lastTripSummary.distance >= rate.min && lastTripSummary.distance <= rate.max) {
                                  if (rate.extraRate && lastTripSummary.distance > 8) {
                                    const extraKm = lastTripSummary.distance - 8;
                                    distanceCost = rate.basePrice! + (extraKm * rate.extraRate);
                                  } else {
                                    distanceCost = rate.price!;
                                  }
                                  break;
                                }
                              }
                            ...
                            const totalFinal = lastTripSummary.costoParadas + distanceCost + waitingCost + sorianaBonus + petCost + servicioEspecialCost;
